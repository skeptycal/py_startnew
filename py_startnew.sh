...
SOURCE  (JSON format) If no SOURCE is given, py_startnew looks for the default
...
OPTIONS     -h  Help ... provide operational information (this screen)
...
# curl -fo .gitignore https://www.gitignore.io/api/c,r,go,qt,web,vue,gis,vim,gpg,lua,osx,node,perl,lamp,rust,java,hugo,hexo,test,text,venv,data,cuda,ruby,latex,pydev,batch,emacs,perl6,vuejs,flask,csharp,python,django,jekyll,dropbox,android,database,assembler,dotfilessh,virtualenv,powershell,dotsettings,pycharm+all,sublimetext,apachehadoop,visualstudio,androidstudio,microsoftoffice,jupyternotebook,jupyternotebooks,visualstudiocode
...
# Parameters:
#
#   optional [start.json] file; if no name is given, setup is interactive
#
...
10     FTP accept failed. While waiting for the  server  to  connect  back
       when an active FTP session is used, an error code was sent over the
